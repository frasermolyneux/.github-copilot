...
description: Align the repository's Deploy Dev GitHub Actions workflow with the standardized format, ensuring that it includes appropriate build, Terraform plan-and-apply, and deployment jobs based on the project's contents.
---
...
Review the existing `.github/workflows/deploy-dev.yml` file in the repository. If it does not exist, create a new one with the standardized configuration for the Deploy Dev workflow. If it does exist, update it to match the standardized configuration, adjusting job templates and directories as needed based on the project contents.

## Deploy Dev

All repositories that have deployable components should have a standardized deploy dev workflow `.github/workflows/deploy-dev.yml` to allow manual deployment to the Development environment. Depending on the project type, use the appropriate combination of build, Terraform, and deployment templates from the `frasermolyneux/actions` GitHub repository.
