...
- Maps feature-owned Playwright is green for the package version running in prd; portal-web host-composition Playwright parity is confirmed with the flag on.
...
3. **Validate** the host (build/test/format + characterization/thin host-composition Playwright) with Maps now unconditional. The feature-repository suite remains the permanent owner of Maps workflows.
...
- Maps served entirely from `portal-feature-maps`; feature-owned Playwright remains green; build/test/format + characterization/thin host-composition Playwright green in the relevant hosts.
